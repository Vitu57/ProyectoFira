...
if (url.match('form_update_excursiones.php')||url.match('form_copy_excursiones.php')) {
    window.onload = asyncCall(1);
}else{
    window.onload = asyncCall(2)
}
...
//const promises = [select_etapa(), select_enum(), select_enum2(), select_enum3(), select_transport()];
async function asyncCall(form_num) {
    console.log('calling');
    await Promise.all([select_etapa(), select_enum(), select_enum2(), select_enum3(), select_transport()]);
    console.log("todo ok");
    if (form_num==1) {
        mostrar_excursion()
    }   
...
function select_enum2() {
    /*código a implementar*/
    return new Promise(function (resolve, reject) {
        lista2 = document.getElementById("ambit_activitat");;
        var ajax3 = objetoAjax();
        var option2;
        ...
        ajax3.onreadystatechange = function () {
            if (ajax3.readyState == 4) {
                if (ajax3.status == 200) {
                    ...
                    //var myJSON = JSON.stringify(this.responseText);
                    option2 = "";
                    option2 += '<option value="0">Escollir una opció</option>';
                    for (var j = 0; j < respuesta.length; j++) {
                        option2 += '<option>' + respuesta[j] + '</option>';
                        //console.log(option);
                    }
                    lista2.innerHTML = option2;
                    //console.log(lista.innerHTML = option);
                    //*select_enum3();
                    resolve('resolved');
                } else {
...
function select_enum3() {
    /*código a implementar*/
    return new Promise(function (resolve, reject) {
        lista3 = document.getElementById("jornada_activitat");
        var ajax3 = objetoAjax();
        var option3;
        ...
        ajax3.onreadystatechange = function () {
            if (ajax3.readyState == 4) {
                if (ajax3.status == 200) {
                    ...
                    //var myJSON = JSON.stringify(this.responseText);
                    option3 = "";
                    option3 += '<option value="0">Escollir una opció</option>';
                    for (var j = 0; j < respuesta.length; j++) {
                        option3 += '<option>' + respuesta[j] + '</option>';
                        //console.log(option);
                    }
                    lista3.innerHTML = option3;
                    //console.log(lista.innerHTML = option);
                    resolve('resolved');
                } else {
                    reject(Error(ajax3.statusText));
