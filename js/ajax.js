...
function modal_admin_user(user){
  ...
  document.getElementById("comprobarModal").value=0;
    ...
    var ajax2=objetoAjax();
    ajax2.open("POST", "../services/consulta_admin_prof.php", true);
    ajax2.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
    ajax2.send("user="+user);
    ajax2.onreadystatechange=function() {
    if (ajax2.readyState==4 && ajax2.status==200) {
    var respuesta=JSON.parse(this.responseText);


           var tabla = '<h2>Eliminar Clase</h2><form action="../services/eliminar_clase_profe.php" method="POST">';
                    

            tabla += '<input type="hidden" id="id_user" name="id_user" value="'+user+'">';

                tabla += '<select id="clase" name="clase">'; 
                   
                   for (var i = 0; i < respuesta.length; i++) {
                    tabla += '<option>' + respuesta[i]+ '</option>';
                    
                    }
                    
                    tabla += '</select><input style="margin:1%;" type="submit" value="Eliminar">';
...
function modal_admin_user2(user){
  ...
  document.getElementById("comprobarModal").value=0;
    ...
    var ajax2=objetoAjax();
    ajax2.open("POST", "../services/consulta_admin_prof2.php", true);
    ajax2.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
    ajax2.send("user="+user);
    ajax2.onreadystatechange=function() {
    if (ajax2.readyState==4 && ajax2.status==200) {
           ...
           var tabla = '<h2>Afegir Clase</h2><form action="eliminar_clase_profe.php" method="POST" onsubmit="eliminar_clase_profe();return false>';
            ...
            tabla += '<input type="hidden" name="id_user" value="'+user+'">';
                ...
                tabla += '<select name="clase">'; 
                   ...
                   tabla += '<input style="margin:1%;" type="submit" value="Afegir">';
